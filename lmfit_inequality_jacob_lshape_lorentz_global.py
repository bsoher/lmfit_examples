"""
LMFIT Using Inequality Constraint and Jacobian - Global Lorentzian lshape
-------------------------------------------------------------------------------
...
This model uses: global lorentzian decay for all peaks
...
def sum_gaussians(pars, t):
    ...
    lshape = np.exp(-t/pars['wid1'])
    model  = pars['amp1'] * np.exp(-1j * pars['cen1']*2*np.pi*t) * lshape
    model += pars['amp2'] * np.exp(-1j * pars['cen2']*2*np.pi*t) * lshape
    model += pars['amp3'] * np.exp(-1j * pars['cen3']*2*np.pi*t) * lshape
...
def dfunc(pars, *args, **kwargs):
    ...
    lshape = np.exp(-t / pars[6])
    gauss1 = np.exp(-1j* pars[3]         *2*np.pi*t)
    gauss2 = np.exp(-1j*(pars[3]+pars[4])*2*np.pi*t)
    gauss3 = np.exp(-1j* pars[5]         *2*np.pi*t)
    all_gauss = pars[0] * gauss1 + pars[1] * gauss2 + pars[2] * gauss3
    # calculate the 9 partial derivatives
    pder.append(gauss1 * lshape)
    pder.append(gauss2 * lshape)
    pder.append(gauss3 * lshape)
    pder.append(-1j*2*np.pi*t * pars[0] * gauss1 * lshape)
    pder.append(-1j*2*np.pi*t * pars[1] * gauss2 * lshape)
    pder.append(-1j*2*np.pi*t * pars[2] * gauss3 * lshape)
    pder.append((t / (pars[6]**2)) * all_gauss * lshape)
