"""
LMFIT Using Inequality Constraint and Jacobian - Indiv Lorentzian lshape
-------------------------------------------------------------------------------
...
This model uses: individual lorentzian decay for each peak
...
def sum_gaussians(pars, t):
    ...
    model  = pars['amp1'] * np.exp(-1j * pars['cen1']*2*np.pi*t) * np.exp(-t/pars['wid1'])
    model += pars['amp2'] * np.exp(-1j * pars['cen2']*2*np.pi*t) * np.exp(-t/pars['wid2'])
    model += pars['amp3'] * np.exp(-1j * pars['cen3']*2*np.pi*t) * np.exp(-t/pars['wid3'])
...
def dfunc(pars, *args, **kwargs):
    ...
    gauss1 = np.exp(-1j* pars[3]         *2*np.pi*t) * np.exp(-t/pars[6])
    gauss2 = np.exp(-1j*(pars[3]+pars[4])*2*np.pi*t) * np.exp(-t/pars[7])
    gauss3 = np.exp(-1j* pars[5]         *2*np.pi*t) * np.exp(-t/pars[8])
    ...
    pder.append((t / (pars[6]**2)) * pars[0] * gauss1)
    pder.append((t / (pars[7]**2)) * pars[1] * gauss2)
    pder.append((t / (pars[8]**2)) * pars[2] * gauss3)
